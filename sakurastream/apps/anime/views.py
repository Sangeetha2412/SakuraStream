...
def home(request):
    trending = Anime.objects.filter(is_trending=True).order_by('-popularity')[:12]
    ...
    latest = Anime.objects.filter(status='airing').order_by('-updated_at')[:12]
    upcoming = Anime.objects.filter(status='upcoming').order_by('aired_from')[:8]
    featured = Anime.objects.filter(is_featured=True).first()
    editor_picks = Anime.objects.filter(is_editor_pick=True)[:6]
    collections = Collection.objects.filter(is_featured=True)[:4]
